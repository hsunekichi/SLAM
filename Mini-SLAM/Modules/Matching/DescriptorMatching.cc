...
int searchForInitializaion(Frame& refFrame, Frame& currFrame, int th, vector<int>& vMatches, std::vector<cv::Point2f>& vPrevMatched){
    ...
    for(size_t i = 0; i < vRefKeys.size(); i++){
        ...
        float radius = 100.0f * currFrame.getScaleFactor(vRefKeys[i].octave);

        // Determine the allowed scale levels for the search.
        int minLevel = vRefKeys[i].octave - 1;
        int maxLevel = vRefKeys[i].octave + 1;

        //Clear previous matches
        vIndicesToCheck.clear();

        currFrame.getFeaturesInArea(vRefKeys[i].pt.x, vRefKeys[i].pt.y, radius, minLevel, maxLevel, vIndicesToCheck);

        // Initialize best distance with the threshold and no match found.
        int bestDist = th;
        int bestIdx = -1;

        // Loop over each keypoint in the current frame.
        for(auto j : vIndicesToCheck){
            // Compute Hamming distance between the two descriptors.
            int dist = HammingDistance(refDesc.row(i), currDesc.row(j));

            // If this distance is the smallest so far and under the threshold, update best match.
            if (dist < bestDist) {
                bestDist = dist;
                bestIdx = j;
            }
        ...
        // If a valid match is found, record it.
        if (bestIdx != -1) {
            vMatches[i] = bestIdx;
            nMatches++;
        }
...
int guidedMatching(Frame& refFrame, Frame& currFrame, int th, std::vector<int>& vMatches, int windowSizeFactor){
    ...
    for(size_t i = 0; i < vRefMapPoints.size(); i++){
        ...
        float radius = 100 * windowSizeFactor * currFrame.getScaleFactor(nLastOctave);
...
int searchWithProjection(Frame& currFrame, int th, std::vector<std::shared_ptr<MapPoint>>& vMapPoints){
    ...
    for(shared_ptr<MapPoint> pMP : vMapPoints){
        ...
        for(auto j : vIndicesToCheck)
        {
            // If the feature is already matched, skip it
            if(currFrame.getMapPoint(j))
                continue;
                

            int dist = HammingDistance(pMP->getDescriptor().row(0), currFrame.getDescriptors().row(j));
